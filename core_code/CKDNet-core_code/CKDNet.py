...
from .position_encoding import build_position_encoding    # added by X.Xu
...
class CKDNet(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_queries, 
        ...
        self.Hypergraph = HyperACE(c1=256, c2=256, num_hyperedges=3)
        # self.Hypergraph = HyperACE(c1=64, c2=128, num_hyperedges=8)

        self.FullPAD_Tunnel_0 = FullPAD_Tunnel()
        self.FullPAD_Tunnel_1 = FullPAD_Tunnel()
        self.FullPAD_Tunnel_2 = FullPAD_Tunnel()
        self.FullPAD_Tunnel_3 = FullPAD_Tunnel()
    ...
    def visualize_feature_map_aferBackbone(self, feat_map, title='', save_path=None, idx=0, max_channels=5):
        """
        可视化特征图（默认可视化 batch 中第 idx 个样本）
        """
        if isinstance(feat_map, torch.Tensor):
            # feat = feat_map.detach().cpu()
            feat = feat_map.detach().cpu()[idx]  # (C, H, W)

        # 取第 idx 个样本，形状 (C, H, W)
        # feat = feat[idx]


        feat_avg = torch.mean(feat, dim=0).numpy()
        plt.figure(figsize=(6, 6))
        plt.imshow(feat_avg, cmap='coolwarm')  # 或 'jet', 'gray''RdBu'
        plt.title(f"{title} (Avg)")
        plt.axis('off')
        save_path = r"E:\PoseCode\ED-Pose-master\logs\backbone_map2_.jpg"
        if save_path:
    ...
    def visualize_feature_map_aferNeck(self, feat_map, title='', save_path=None, idx=0, max_channels=5):
        """
        可视化特征图
        """
        if isinstance(feat_map, torch.Tensor):
        ...
        feat_avg = torch.mean(feat, dim=0).numpy()
        plt.figure(figsize=(6, 6))
        plt.imshow(feat_avg, cmap='coolwarm')  # 或 'jet', 'gray'
        plt.title(f"{title} (Avg)")
        plt.axis('off')
        ...
        for i in range(min(max_channels, feat.shape[0])):
            plt.figure()
            plt.imshow(feat[i], cmap='coolwarm')
            plt.title(f'{title} - Channel {i}')
            plt.axis('off')
            plt.show()
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        """ The forward expects a NestedTensor(嵌套张量), which consists of:
               - samples.tensor: batched images(批量数据), of shape [batch_size x 3 x H x W]
            ...
            It returns a dict with the following elements:
               ...
               - "pred_boxes": The normalized boxes coordinates for all queries, represented as
                               (center_x, center_y, width, height)（中心点x,中心点y,宽, 高）. These values are normalized in [0, 1],
        ...
        features0,_ = self.backbone(samples)

        # 此处features是一个list.  poss和fetures一样
        # features[0]的tensors.shape是4,512,92,164  (B, C, H, W)
        # features[1]的tensors.shape是4,1024,46,82
        # features[2]的tensors.shape是4,2048,23,41

        data = []
        for item in features0:
            data.append(item.tensors.to(item.tensors.dtype))

        #---- Model Refinement ----#
        headout = []
        h5 = self.conv2048256(data[2])      ### (B,2048,H/4,W/4) to (B,256,H/4,W/4)
        h4 = self.conv1024256(data[1])      ### (B,1024,H/2,W/2) to (B,256,H/2,W/2)
        h3 = self.conv512256(data[0])       ### (B,512, H,  W)   to (B,256,H,  W)

        # self.visualize_feature_map_aferNeck(h3, idx=0, title='before')
        h3 = self.BFEM(h3, h5)    # 使用BFEM
        # self.visualize_feature_map_aferNeck(h3, idx=0, title='afer')

        x_list = [h3, h4, h5]
        HyperACE_out = self.Hypergraph(x_list)    # (B,256, H/2,W/2)
        HyperACE_out_h3 = F.interpolate(HyperACE_out, size=h3.shape[2:], mode='bilinear', align_corners=False)
        HyperACE_out_h4 = F.interpolate(HyperACE_out, size=h4.shape[2:], mode='bilinear', align_corners=False)
        HyperACE_out_h5 = F.interpolate(HyperACE_out, size=h5.shape[2:], mode='bilinear', align_corners=False)

        h5Up = self.upsample(h5)          # (B, 256,H/4,W/4) to (B, 256,H/2,W/2)
        if h4.shape != h5Up.shape:
            assert h4.shape != h5Up.shape

        h4 = self.FullPAD_Tunnel_0([h4, HyperACE_out_h4]) # HGAEM
        h45 = torch.cat((h5Up, h4), dim=1)      # ConCat
        h45 = self.Repc3(h45)
        h4c = self.conv256256(h45)
        h4Up = self.upsample(h4c)         # (B, 256,H/2,W/2) to (B,256,H,W)

        assert h3.shape[3] == h4Up.shape[3], f"Width mismatch: {h3.shape[3]} vs {h4Up.shape[3]}"
        assert h3.shape[2] == h4Up.shape[2], f"Height mismatch: {h3.shape[2]} vs {h4Up.shape[2]}"
        assert h3.shape == h4Up.shape, f"Shape mismatch: {h3.shape} vs {h4Up.shape}"

        h3 = self.FullPAD_Tunnel_1([h3, HyperACE_out_h3]) # HGAEM
        h34 = torch.cat((h3, h4Up), dim=1)      # ConCat
        h34 = self.Repc3(h34)
        headout.append(h34)         # headout 1

        Y4 = self.conv32(h34)
        if h4c.shape != Y4.shape:
            assert h4c.shape != Y4.shape

        Y4 = torch.cat((h4c, Y4), dim=1)        # ConCat
        F4 = self.Repc3(Y4)
        headout.append(F4)          # headout 2

        F4 = self.conv32(F4)

        h5 = self.FullPAD_Tunnel_1([h5, HyperACE_out_h5]) # HGAEM
        Y5 = torch.cat((F4, h5), dim=1)         # ConCat
        F5 = self.Repc3(Y5)
        headout.append(F5)          # headout 3
        # 图展示
        # self.visualize_feature_map_aferNeck(headout[0], idx=0, title='aferNeck Feature0 Layer 0')

        poss = []

        for i in range(len(features0)):
            features0[i].tensors = headout[i]
        ...
        srcs = []
        masks = []
        for l, feat in enumerate(features):    # 特征投影与处理(多尺度)
            src, mask = feat.decompose()
            srcs.append(src)   # YGY is “srcs.append(src)”    #
            # srcs.append(self.input_proj[l](src))    # YGY is “srcs.append(src)”    #
            masks.append(mask)
        ...
        if self.num_feature_levels > len(srcs):
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                if l == _len_srcs:
                    ...
                    # src = self.conv32(features[-1].tensors)；   src = self.AIFI(src)
                else:
        ...
        if self.dn_number > 0 or targets is not None:    # DN准备    # 构建DN（Denoising）输入（如果启用）
        ...
        # Transformer
        # 输入多尺度特征、掩码、位置编码，输出包含关键查询向量（hs）、位置参考（reference）等关键表示。
        ...
        for dec_lid, (layer_ref_sig, layer_bbox_embed,layer_cls_embed, layer_hs) in enumerate(zip(reference[:-1], self.bbox_embed,self.class_embed, hs)):
            if dec_lid < self.num_box_decoder_layers:    # num_box_decoder_layers 之前的层：常规边框回归
            ...
            else:                                       # num_box_decoder_layers 之后的层：处理 DN + 正常queries 情况
